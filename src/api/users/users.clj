...
(defn create-request [user_origin user_destiny]
  (let [request {:source user_origin :destiny user_destiny :status "Pending"}]
    request))
    ...
    (if (or (nil? user_destiny) (= (:active user_destiny) false))
      ...
      (do
        (if (not= nil (some (partial = (:username user_destiny)) (:friends user_origin)))
          ...
          (let [request (create-request (:username user_origin) (:username user_destiny))
                receiver (merge user_destiny {:requests (conj (:requests user_destiny) request)})]
            (update-user (:_id user_destiny) receiver)
            (send-email email-conn {:from user_mail :to (:email user_destiny) :subject "Friend request"
                                    :body (str (:username user_origin) " sent you an invitation to be friends. Go check it out!")})
            (success (dissoc user_origin :_id))))))))

(defn accept-request [{sender :sender receiver :receiver}]
  (let [user_sender (find-user {:username sender})
        user_receiver (find-user {:username receiver})
        friends_sender (conj (:friends user_sender) receiver)
        friends_receiver (conj (:friends user_receiver) sender)]
    (update-user (:_id user_sender) (merge user_sender {:friends friends_sender}))
    (update-user (:_id user_receiver) (merge user_receiver {:friends friends_receiver}
                                             {:requests (remove #{{:source sender :destiny receiver :status "Pending"}} (:requests user_receiver))}))
